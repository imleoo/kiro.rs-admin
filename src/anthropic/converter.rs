...
pub fn normalize_model_id(model: &str) -> String {
    map_model(model).unwrap_or_else(|| model.trim().to_string())
}
...
/// 对未进入静态列表的「未来」Claude 版本按 major.minor 推断上下文窗口：
/// >= 4.6 视为 1M（延续 Opus/Sonnet 4.6 起的上游升级），否则回退 200K。
...
/// （静态列表或 `claude_context_window` 的推断规则）。
pub fn get_context_window_size(model: &str) -> i32 {
    ...
    match map_model(model) {
        ...
        Some(mapped) if mapped.to_ascii_lowercase().starts_with("deepseek-") => 164_000,
        Some(mapped) if mapped.to_ascii_lowercase().starts_with("minimax-") => 196_000,
        Some(mapped) if mapped.to_ascii_lowercase().starts_with("qwen") => 256_000,
        Some(mapped) if mapped.to_ascii_lowercase().starts_with("claude-") => {
            claude_context_window(&mapped.to_ascii_lowercase()).unwrap_or(200_000)
        }
        _ => 200_000,
    }
}
...
mod tests {
    ...
    #[test]
    fn test_get_context_window_size_native_kiro_models() {
